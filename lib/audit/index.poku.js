...
import { writeBinary } from "../helpers/protobom.js";
import {
...
describe("loadInputBoms()", () => {
  it("loads valid BOMs from a directory and skips unrelated JSON files", async () => {
    ...
    try {
      const inputBoms = await loadInputBoms({ bomDir: tmpDir });
  ...
  it("loads protobuf BOMs from a directory", async () => {
    const tmpDir = mkdtempSync(path.join(os.tmpdir(), "cdx-audit-proto-"));
    const bomPath = path.join(tmpDir, "bom.cdx");
    ...
    try {
      ...
      assert.strictEqual(inputBoms.length, 1);
      assert.strictEqual(inputBoms[0].source, bomPath);
      assert.strictEqual(inputBoms[0].bomJson?.specVersion, "1.7");
      assert.strictEqual(
        inputBoms[0].bomJson?.metadata?.component?.name,
        "demo-app",
      );
    } finally {
      rmSync(tmpDir, { force: true, recursive: true });
  ...
  it("fails fast for protobuf BOMs discovered in a bom directory when protobuf support is unavailable", async () => {
    const tmpDir = mkdtempSync(
      path.join(os.tmpdir(), "cdx-audit-proto-missing-"),
    );
    const bomPath = path.join(tmpDir, "bom.cdx");
    writeFileSync(bomPath, "not-used");
    const protoSupportError = new Error(
      "cdx-audit protobuf BOM input requires the optional '@appthreat/cdx-proto' dependency. Install it or use a build that bundles protobuf support.",
    );
    const { loadInputBoms: loadInputBomsMocked } = await esmock("./index.js", {
      "../helpers/protobomLoader.js": {
        importProtobomModule: sinon.stub().rejects(protoSupportError),
        isProtoBomPath: (filePath) =>
          `${filePath}`.toLowerCase().endsWith(".cdx"),
      },
    });

    try {
      await assert.rejects(
        () => loadInputBomsMocked({ bomDir: tmpDir }),
        /requires the optional '@appthreat\/cdx-proto' dependency/u,
      );
    } finally {
      rmSync(tmpDir, { force: true, recursive: true });
    }
...
describe("auditTarget() cache resume", () => {
  it("reuses a cached child SBOM from the workspace without resolving or regenerating source", async () => {
    ...
    try {
      ...
      assert.strictEqual(auditBomStub.callCount, 1);
      assert.deepStrictEqual(auditBomStub.firstCall.args[0], cachedBom);
    } finally {
