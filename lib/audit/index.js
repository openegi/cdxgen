...
import {
  ...
  isProtoBomPath,
} from "../helpers/protobomLoader.js";
import {
 ...
 * @returns {Promise<object>} parsed CycloneDX BOM
 */
export async function loadBomFile(bomPath) {
  const resolvedPath = resolve(bomPath);
  let bomJson;
  try {
    if (isProtoBomPath(resolvedPath)) {
      const { readBinary } = await importProtobomModule(
        DIRECT_AUDIT_TOOL_NAME,
        "protobuf BOM input",
        import.meta.url,
      );
      bomJson = readBinary(resolvedPath, true);
    } else {
...
/**
 * Recursively list JSON and protobuf BOM files under a BOM directory.
 *
...
export function listBomFiles(bomDir) {
  ...
  while (queue.length) {
    ...
    for (const entry of entries) {
      ...
      if (
        entry.isFile() &&
        (entry.name.endsWith(".json") || isProtoBomPath(entry.name))
      ) {
 ...
 * @returns {Promise<{ source: string, bomJson: object }[]>} loaded input BOMs
 */
export async function loadInputBoms(options) {
  const inputBoms = [];
  if (options.bom) {
    inputBoms.push({
      bomJson: await loadBomFile(options.bom),
  ...
  if (options.bomDir) {
    ...
    for (const bomFile of bomFiles) {
      try {
        inputBoms.push({
          bomJson: await loadBomFile(bomFile),
          source: bomFile,
        });
      } catch (error) {
        if (isProtoBomPath(bomFile)) {
          throw error;
        }
        console.warn(
          `Skipping non-CycloneDX BOM file '${bomFile}': ${error.message}`,
        );
      }
    }
  }
...
async function loadCachedChildBom(workspaceDir, target) {
  if (!workspaceDir) {
    return undefined;
  }
  ...
  try {
    const bomJson = await loadBomFile(cachePaths.sourceBomFile);
    let metadata = {};
...
export async function auditTarget(target, options) {
  ...
  try {
    const cachedChildBom = await loadCachedChildBom(
...
export async function runAudit(options) {
  const inputBoms = await loadInputBoms(options);
